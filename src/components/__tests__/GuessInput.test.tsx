...
describe("GuessInput", () => {
  ...
  it("renders input and button", () => {
    render(
      <GuessInput gameState={gameState} setGameState={mockSetGameState} />
    );
  ...
  it("shows error for out of range number", async () => {
    ...
    expect(
      await screen.findByText(/Please enter a number between 1 and 100/)
    ).toBeInTheDocument();
  });
  ...
  it("disables input when game is over", () => {
    ...
    render(
      <GuessInput gameState={gameOverState} setGameState={mockSetGameState} />
    );
